...
class DeeplinkService {
  ...
  void _handleInAppUri(Uri uri) {
    ...
    try {
      ...
      debugPrint('[DeeplinkService] Jadwal navigasi /payment/deeplink — ${data.merchantName} ${data.amount}');
      WidgetsBinding.instance.addPostFrameCallback((_) {
        debugPrint('[DeeplinkService] router.go("/payment/deeplink") dieksekusi');
        _router.go('/payment/deeplink', extra: data);
      });
    } on FormatException catch (e) {
      debugPrint('[DeeplinkService] Format error: ${e.message}');
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _router.go('/payment/deeplink', extra: e.message);
      });
    }
  }
